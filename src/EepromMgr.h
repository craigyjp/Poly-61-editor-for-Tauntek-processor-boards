...
#define EEPROM_SAVE_EDITOR_ALL 13
...
boolean getEncoderAccelerate() {
  ...
  if (accelerate < 0 || accelerate > 1)return true; //If EEPROM has no encoder direction stored
...
boolean getSaveEditorAll() {
  byte sea = EEPROM.read(EEPROM_SAVE_EDITOR_ALL); 
  if (sea < 0 || sea > 1)return true;
  return sea ? true : false;
}

void storeSaveEditorAll(byte seaupdate)
{
  EEPROM.write(EEPROM_SAVE_EDITOR_ALL, seaupdate);
  EEPROM.commit();
}
